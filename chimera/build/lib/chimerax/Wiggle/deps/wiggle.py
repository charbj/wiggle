...
class Main(object):
    ...
    def launchEmbedding(self):
        ...
        if self.checkBox_12.checkState() == QtCore.Qt.Checked and self.lineEdit_3.text().isnumeric():
            self.worker = Embedder(
                self.comboBox_3.currentIndex(),
                self.lineEdit_4.text(),
                self.lineEdit_3.text(),
                True)
        else:
            self.worker = Embedder(self.comboBox_3.currentIndex(),
                                 self.lineEdit_4.text(),
                                 self.lineEdit_3.text(),
                                 False
            )
    ...
    def launchKmeans(self):
        ...
        self.thread = QtCore.QThread()
        # Step 3: Create a worker object
        self.worker = Clusterer(self.spinBox_2.value(), self.comboBox_4.currentIndex())
        # Step 4: Move worker to the thread
        self.worker.moveToThread(self.thread)
        # Step 5: Connect signals and slots
    ...
    def update_plot(self): ###Try break this up or move to a seperate thread for smoothness and speed.
        ...
        def map_to_colour(slice):
            ...
            cm = pg.colormap.get(userColourOptions[self.comboBox_5.currentIndex()], source='matplotlib')
            return cm[slice]
    ...
    def return_pressed(self):
        ...
        vol = volume_from_grid_data(ArrayGridData(miniDRGN.vol, name='cryoDRGN vol'), self.wiggle)
        vol.show()
...
class Embedder(QtCore.QObject):
    ...
    def __init__(self, technique_index, data, frac, subset: bool, ):
    ...
    def run_embedding(self): ###Re implement as above, too much redundancy.
        ...
        global data
        if self.do_subset:
            data = self.load_data(self.data_path, do_subset=True, fraction=int(self.fraction))
            self.msg.emit("Subsampling latent space to %s \n -------------------------" % int(self.fraction))
        else:
            data = self.load_data(self.data_path, do_subset=False)
...
class Clusterer(QtCore.QObject):
    ...
    msg = QtCore.pyqtSignal(str)
    status = QtCore.pyqtSignal(bool)
    def __init__(self, clusters, method):
    ...
    def run_clustering(self):
        ...
        ward = cluster.AgglomerativeClustering(n_clusters=self.clusters, linkage='ward',
        ...
        average_linkage = cluster.AgglomerativeClustering(linkage="average", affinity="cityblock",
                                                          n_clusters=self.clusters, connectivity=connectivity)
