...
class params:
    def __init__(self, config, weights, apix, flip):
        self.Apix = apix
        self.D = None
...
class miniDRGN:
    def __init__(self, config, weights, apix, flip):
        ...
        self.apix = apix
        self.flip = flip
        self.args = params(self.config, self.weights, self.apix, self.flip)
    ...
    def generate(self, z):
        ...
        if args.downsample:
            ...
            vol = self.model.decoder.eval_volume(self.lattice.get_downsample_coords(args.downsample+1),
                                            args.downsample+1, extent, self.norm, z)
        else:
            vol = self.model.decoder.eval_volume(self.lattice.coords, self.lattice.D, self.lattice.extent, self.norm, z)
