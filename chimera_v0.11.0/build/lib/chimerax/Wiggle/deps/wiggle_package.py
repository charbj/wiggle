...
class PackIt:
    ...
    def read_cryodrgn(self):
        ...
        if not os.path.isfile(self.args.output):
            np.savez_compressed(self.args.output, config=config, weights=weights, z_space=z_space)
        else:
            print(f'{self.args.output} already exists. Exiting...')
